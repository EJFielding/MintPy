...
# routines from MintPy
from mintpy.utils import readfile, plot as pp

# routines copied from mintpy/save_gmt.py 
...
####################################################################################
def convert_geom_gmt(inps):
    
    if not inps.file:
        inps.file = 'geo/geo_geometryRadar.h5'  

    # Read incidence angle data from original MintPy to get all the attributes
    dset = 'incidenceAngle'
    dataI, atr = readfile.read(inps.file, datasetName=dset) 

    dset = 'azimuthAngle'
    dataA, atr1 = readfile.read(inps.file, datasetName=dset) 

    # calculate components
    U=np.cos(np.deg2rad(dataI))
    N=np.sin(np.deg2rad(dataI)) * np.sin(np.deg2rad(dataA+90))
    E=np.sin(np.deg2rad(dataI)) * np.cos(np.deg2rad(dataA+90))

    outEname = 'LOSeast.grd'
    outfile = write_grd_file(E, atr, outEname)

    outNname = 'LOSnorth.grd'
    outfile = write_grd_file(N, atr, outNname)

    outUname = 'LOSup.grd'
    outfile = write_grd_file(U, atr, outUname)
